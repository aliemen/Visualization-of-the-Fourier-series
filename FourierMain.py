...
def get_fourier_coeff(func, T=[0, 1], N=4):
    ...
    for k, ind in enumerate(indizes):
        ...
        coeff[k] = solver.get_approximation(100, n_gauss_param=6)
...
def get_fourier_vector_line(t, coeff, ind):
    ...
    ret_line = np.empty(len(coeff)+1, dtype=complex) # stores all complex values for the line
    ret_line[0] = 0
...
def get_desmos_string(coeff, ind): # returns 2D vector with sin and cos-funcs to copy into desmos
    ...
    return f"( {real_string} , {imag_string} )".replace("       ", "").replace("     ", "").replace("    ", "").replace("   ", "").replace("  ", "")
...
def main():    
    ...
    n_eval = 1000 # number of evaluations on the function (normally "25*animation_time" should be the most efficient)
    ...
    if not only_fourier_calc:
        ...
        anim = fourier_animation(figure_data, fourier_data, plot_reference=False, handler=handler, plot_whole_approximation=False,
        ...
        if save_video:
            ...
            writer = Writer(fps=25, metadata=dict(artist='Me'), bitrate=20000)
